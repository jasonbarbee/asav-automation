...
def ucs_add_vlan_to_vnic(module):
    ...
    for FI in FILIST:
        try:
            mo = VnicLanConnTempl(parent_mo_or_dn="org-root", redundancy_pair_type="none", nw_ctrl_policy_name="", descr="", stats_policy_name="default", admin_cdn_name="", switch_id=FI, pin_to_group_name="", mtu="1500", policy_owner="local", peer_redundancy_templ_name="", templ_type="updating-template", qos_policy_name="", ident_pool_name="", cdn_source="vnic-name", name=vnic_template)
...
def ucs_remove_vlan_from_vnic(module):
    ...
    for FI in FILIST:
        try:
            mo_1 = ucsm.handle.query_dn("org-root/lan-conn-templ-test-template/if-"+vlan_name)
